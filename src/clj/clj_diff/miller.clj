...
(defn- edits
  ...
  ([a b p delta graph opts]
   (let [next-fn (fn next-fn [p x k] (next-edit a b graph delta p x k opts))]
    (loop [edits '()
...
(defn seq-diff
  ...
  ([a b opts]
   (let [[a b] (vectorize a b)
         [[p delta graph] a* b*] (order->ses a b opts)
         edits (edits a* b* p delta graph opts)]
